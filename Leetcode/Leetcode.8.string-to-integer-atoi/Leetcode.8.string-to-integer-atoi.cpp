...
public:
    int myAtoi(string s) {
        long answer = 0; // Pervent overflow
        int idx = 0;
        bool positive = true;
        const int kLength = s.length();
        while (idx < kLength && s.at(idx) == ' ') {
            idx++;
        }
        if (idx < kLength && s.at(idx) == '-') { // If there is a sign char
            idx++;
            positive = false;
        } else if (idx < kLength && s.at(idx) == '+') {
            idx++;
        }
        while (idx < kLength && s.at(idx) >= '0' && s.at(idx) <= '9' ) {
            if (positive) {
                answer = answer * 10 + (s.at(idx) - '0');
            } else {
                answer = answer * 10 - (s.at(idx) - '0');
            }
