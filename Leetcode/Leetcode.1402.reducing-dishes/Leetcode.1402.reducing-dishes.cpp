...
public:
    int maxSatisfaction(vector<int>& satisfaction) { // Original One-pass
        ...
        vector<int> sum(kSize, 0);

        int temp(satisfaction.at(0));
        int total(temp);
        sum.at(0) = temp;
        for (int i = 1; i < kSize; ++i) {
            int curr(satisfaction.at(i));
            temp += curr;
            total += (i + 1) * curr;
            sum.at(i) = temp;
        }
        ...
        for (int i(0); i < kSize; ++i) {
            int remove(satisfaction.at(i));
            int drawback(sum.at(end) - sum.at(i));
            // cout << drawback << ' ' << remove << '\n';
            if (drawback > -remove) {
                break;
            } else {
                total -= (remove + drawback);
            }
        }
        return total;
    }
