...
public:
    ...
    void dfs(int idx, int parent) {
        ...
        auto& curr_count = count.at(idx);
        for (int next : graph.at(idx)) {
            if (next == parent) {
                continue;
            }
            dfs(next, idx);
            for (pair<char, int> p : count.at(next)) {
                char label = p.first;
                if (curr_count.count(label) > 0) {
                    curr_count.at(label) += p.second;
                } else {
                    curr_count.insert(pair<char, int>(label, p.second));
                }
            }
        }
        char label = labels.at(idx);
        if (curr_count.count(label) > 0) {
            curr_count.at(label) += 1;
        } else {
            curr_count.insert(pair<char, int>(label, 1));
        }
        ans.at(idx) = curr_count.at(label);
        
    }

    vector<int> countSubTrees(int n, vector<vector<int>>& edges, string labels0) {
        labels = labels0;
        ans.resize(n, 0);
        count.resize(n, {});
        graph.resize(n, {});
