...
public:
    ...
    void set(string key, string value, int timestamp) {
        if (mp.count(key) == 0) {
            mp.insert(pair<string, map<int, string>>(key, {}));
        }
        mp.at(key).insert({timestamp, value});
    }
    
    string get(string key, int timestamp) {
        if (mp.count(key) == 0) return "";
        auto to_return = mp.at(key).upper_bound(timestamp);
        if (to_return == mp.at(key).begin()) {
            return "";
        } else {
            return prev(to_return)->second;
        }
    }
};
