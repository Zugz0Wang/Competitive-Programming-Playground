...
class Solution {
public:
    string helper(string& senate, vector<bool>& alive) { // Independently solved
        int ban_r_next(0);
        int ban_d_next(0);
        int r_count(0);
        int d_count(0);
        int count(0);
        const size_t size(senate.size());
