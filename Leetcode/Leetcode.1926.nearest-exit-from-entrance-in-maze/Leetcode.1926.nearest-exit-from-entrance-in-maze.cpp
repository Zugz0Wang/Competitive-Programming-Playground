...
public:
    int nearestExit(vector<vector<char>>& maze, vector<int>& entrance) {
        const int kRow = maze.size();
        const int kCol = maze.at(0).size();
        bool visit[100][100];
        memset(visit, true, sizeof(visit));
        queue<pair<int,int>> wait;

        visit[entrance.at(0)][entrance.at(1)] = false;
        wait.push(pair<int,int>(entrance.at(0), entrance.at(1)));
        int count = 0;
        while (wait.size() > 0) {
            int amount = wait.size();
            for (int i = 0; i < amount; i++) {
                pair<int, int> curr_node = wait.front();
                wait.pop();
                int x = curr_node.first;
                int y = curr_node.second;
                if (x - 1 >= 0 && visit[x - 1][y] && maze.at(x - 1).at(y) == '.') {
                    if (x - 1 == 0 || y == 0 || y == kCol - 1) {
                        return count + 1;
                    }
                    wait.push(pair<int, int>(x - 1, y));
                    visit[x - 1][y] = false;
                }
                if (x + 1 < kRow && visit[x + 1][y] && maze.at(x + 1).at(y) == '.') {
                    if (x + 1 == kRow - 1 || y == 0 || y == kCol - 1) {
                        return count + 1;
                    }
                    wait.push(pair<int, int>(x + 1, y));
                    visit[x + 1][y] = false;
                }
                if (y - 1 >= 0 && visit[x][y - 1] && maze.at(x).at(y - 1) == '.') {
                    if (x == 0 || x == kRow - 1 || y - 1 == 0) {
                        return count + 1;
                    }
                    wait.push(pair<int, int>(x, y - 1));
                    visit[x][y - 1] = false;
                }
                if (y + 1 < kCol && visit[x][y + 1] && maze.at(x).at(y + 1) == '.') {
                    if (x == 0 || x == kRow - 1 || y + 1 == kCol - 1) {
                        return count + 1;
                    }
                    wait.push(pair<int, int>(x, y + 1));
                    visit[x][y + 1] = false;
