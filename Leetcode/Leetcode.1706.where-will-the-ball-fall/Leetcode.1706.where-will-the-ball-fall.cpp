...
public:
    vector<int> findBall(vector<vector<int>>& grid) {
        ...
        vector<int> to_return = vector<int>(kCol, -1);
        for (int col = 0; col < kCol; col++) {
            int idx = col;
            for (int row = 0; row < kRow; row++) {
                bool direction = grid.at(row).at(idx) > 0;
                if (direction && idx + 1 < kCol && grid.at(row).at(idx + 1) == 1) {
                    idx += 1;
                } else if (!direction && idx > 0 && grid.at(row).at(idx - 1) == -1) {
                    idx -= 1;
                } else {
                    //cout << idx << ' ';
                    idx = -1;
