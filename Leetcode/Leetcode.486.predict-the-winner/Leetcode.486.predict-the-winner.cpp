...
public:
    ...
    int helper(size_t begin, size_t end) {
        ...
        // cout << begin << ' ' << end << '\n';
        int& curr(dp[begin * size + end]);
        if (visited[begin * size + end]) {
            return curr;
        } else {
            visited[begin * size + end] = 1;
        }
        ...
        int left(nums[begin] - helper(begin + 1, end));
        int right(nums[end - 1] - helper(begin, end - 1));
        if (left > right) {
            curr = left;
        } else {
            curr = right;
    ...
    bool PredictTheWinner(vector<int>& nums_v) {
        size = nums_v.size();
        vector<int> dp_v((size + 1) * (size + 1), 0);
        vector<int> visited_v((size + 1) * (size + 1), 0);
        dp = dp_v.data();
        visited = visited_v.data();
        nums = nums_v.data();

        return helper(0, size) >= 0;
    }
};
// @lc code=end
