...
public:
    ...
    bool isScramble(string s1, string s2) {
        ...
        if (mp.count(key) > 0) return mp.at(key);

        const int kSize(s1.size());
        bool flag(false);
        for (int i(1); i < kSize; ++i) {
            const int length2(kSize - i);
            if (isScramble(s1.substr(0, i), s2.substr(0,i)) && 
                isScramble(s1.substr(i,length2), s2.substr(i,length2))) {
                flag = true;
                break;
            }

            if (isScramble(s1.substr(0, length2), s2.substr(i,length2)) && 
                isScramble(s1.substr(length2, i), s2.substr(0,i))) {
                flag = true;
                break;
            }
        }
        mp.insert(pair<string, bool>(key, flag));
        return flag;
    }
};
// @lc code=end
