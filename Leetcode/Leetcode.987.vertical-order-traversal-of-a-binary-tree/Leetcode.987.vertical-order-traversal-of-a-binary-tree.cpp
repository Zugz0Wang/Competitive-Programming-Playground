...
public:
    ...
    void helper (TreeNode* root, int row, int col) {
        if (mp.count(col) > 0) {
            mp.at(col).push_back(Node(row, root->val));
        } else {
            mp.insert(pair<int, vector<Node>>(col, {Node(row, root->val)}));
        }
        if (root->left) helper(root->left, row + 1, col - 1);
        if (root->right) helper(root->right, row + 1, col + 1);
        return;
    }
    vector<vector<int>> verticalTraversal(TreeNode* root) {
        ...
        for (auto& i : mp) {
            ...
            for (Node& j : temp) {
                to_return.at(count).push_back(j.val);
            }
            count++;
        }
        return to_return;
