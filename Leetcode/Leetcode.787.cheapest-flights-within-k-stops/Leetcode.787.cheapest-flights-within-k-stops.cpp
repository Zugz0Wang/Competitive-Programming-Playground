/*
 * @lc app=leetcode id=787 lang=cpp
 *
 * [787] Cheapest Flights Within K Stops
 *
 * https://leetcode.com/problems/cheapest-flights-within-k-stops/description/
 *
 * algorithms
 * Medium (35.86%)
 * Likes:    6476
 * Dislikes: 294
 * Total Accepted:    302.7K
 * Total Submissions: 843.3K
 * Testcase Example:  '4\n[[0,1,100],[1,2,100],[2,0,100],[1,3,600],[2,3,200]]\n0\n3\n1'
 *
 * There are n cities connected by some number of flights. You are given an
 * array flights where flights[i] = [fromi, toi, pricei] indicates that there
 * is a flight from city fromi to city toi with cost pricei.
 * 
 * You are also given three integers src, dst, and k, return the cheapest price
 * from src to dst with at most k stops. If there is no such route, return
 * -1.
 * 
 * 
 * Example 1:
 * 
 * 
 * Input: n = 4, flights = [[0,1,100],[1,2,100],[2,0,100],[1,3,600],[2,3,200]],
 * src = 0, dst = 3, k = 1
 * Output: 700
 * Explanation:
 * The graph is shown above.
 * The optimal path with at most 1 stop from city 0 to 3 is marked in red and
 * has cost 100 + 600 = 700.
 * Note that the path through cities [0,1,2,3] is cheaper but is invalid
 * because it uses 2 stops.
 * 
 * 
 * Example 2:
 * 
 * 
 * Input: n = 3, flights = [[0,1,100],[1,2,100],[0,2,500]], src = 0, dst = 2, k
 * = 1
 * Output: 200
 * Explanation:
 * The graph is shown above.
 * The optimal path with at most 1 stop from city 0 to 2 is marked in red and
 * has cost 100 + 100 = 200.
 * 
 * 
 * Example 3:
 * 
 * 
 * Input: n = 3, flights = [[0,1,100],[1,2,100],[0,2,500]], src = 0, dst = 2, k
 * = 0
 * Output: 500
 * Explanation:
 * The graph is shown above.
 * The optimal path with no stops from city 0 to 2 is marked in red and has
 * cost 500.
 * 
 * 
 * 
 * Constraints:
 * 
 * 
 * 1 <= n <= 100
 * 0 <= flights.length <= (n * (n - 1) / 2)
 * flights[i].length == 3
 * 0 <= fromi, toi < n
 * fromi != toi
 * 1 <= pricei <= 10^4
 * There will not be any multiple flights between two cities.
 * 0 <= src, dst, k < n
 * src != dst
 * 
 * 
 */
#include <vector>
using namespace std;
// @lc code=start
class Solution {
public:
    struct node {
        int stop;
        int src;
        int cost;
        node(int s, int sr, int c): stop(s), src(sr), cost(c) {}
    };
    int findCheapestPrice(int n, vector<vector<int>>& flights, int src, int dst, int k) {
        // Adapted from discussion, changed the pair<pair> design into struct.
        vector<vector<pair<int, int>>> graph(n);
        for (auto flight : flights) {
            graph.at(flight.at(0)).push_back({flight.at(1), flight.at(2)});
        }
        vector<int> dis(n, -1);
        queue<node> wait;
        wait.push(node(0, src, 0));
        dis.at(src) = 0;
        while (!wait.empty()) {
            node curr = wait.front();
            wait.pop();
            if (curr.stop > k) continue;
            for (auto next: graph.at(curr.src)) {
                int next_node = next.first;
                int next_cost = next.second;
                if ((dis.at(next_node) < 0 || next_cost + curr.cost < dis.at(next_node)) && curr.stop <= k) {
                    dis.at(next_node) = next_cost + curr.cost;
                    wait.push(node(curr.stop + 1, next_node, dis.at(next_node)));
                }
            }
        }
        return dis.at(dst);

    }
};
// @lc code=end

