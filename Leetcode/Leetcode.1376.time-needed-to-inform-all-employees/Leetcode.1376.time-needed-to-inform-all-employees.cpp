...
public:
    ...
    int helper(int root) {
        const vector<int>& nexts(g.at(root));
        int sub_time(0);
        for (size_t next : nexts) {
            int next_time(helper(next));
            if (next_time > sub_time) {
                sub_time = next_time;
            }
        }
        return sub_time + informTime.at(root);
    }

    int numOfMinutes(int n, int headID, vector<int>& manager, vector<int>& informTime_) {
        informTime = informTime_;
        g.resize(n);
        const int size(manager.size());
        for (int i(0); i < size; ++i) {
            int curr_manager(manager.at(i));
            if (curr_manager >= 0) {
                g.at(curr_manager).push_back(i);
            }
        }
