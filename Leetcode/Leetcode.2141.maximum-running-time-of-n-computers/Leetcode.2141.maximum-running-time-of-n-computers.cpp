...
public:
    ...
    bool isValid(long long time) {
        long long total(0);
        for (size_t i(0); i < size; ++i) {
            total += min(time, (long long)batteries[i]);
        }
        return total >= time * n;
    }

    long long maxRunTime(int n_, vector<int>& batteries_) {
        n = n_;
        size = batteries_.size();
        batteries = batteries_.data();
        long long begin(0);
        long long end(10);
        for (size_t i(0); i < size; ++i) {
            end += batteries[i];
        ...
        // cout << begin << ' ' << end << '\n';
        while (begin < end) {
            long long mid_time((begin + end) / 2);
